...
TEST_F(CppClassTest, classW_MethodsTest) {
    ...
    ASSERT_EQ(pckg->getPackagedElements().get(1).getElementType(), ElementType::CLASS);
    Class& clazz = pckg->getPackagedElements().get(1).as<Class>();
    ASSERT_EQ(clazz.getOwnedOperations().size(), 3);
    Operation& foo = clazz.getOwnedOperations().front();
    ASSERT_EQ(foo.getVisibility(), VisibilityKind::PRIVATE);
    ASSERT_EQ(foo.getOwnedParameters().size(), 0);
    Operation& bar = clazz.getOwnedOperations().get(1);
    ...
    Operation& tt = clazz.getOwnedOperations().get(2);
...
TEST_F(CppClassTest, predefinedTypeTest) {
    ...
    Property& ptrProp = bar.getOwnedAttributes().get(1);
    ...
    ASSERT_EQ(ptrEnd.getType()->getID(), bar.getID());
    ASSERT_EQ(ptrAssoc.getMemberEnds().size(), 2);
    ASSERT_EQ(ptrAssoc.getMemberEnds().front().getID(), ptrProp.getID());
    Property refProp = bar.getOwnedAttributes().get(2);
    ASSERT_EQ(refProp.getName(), "f_ref");
