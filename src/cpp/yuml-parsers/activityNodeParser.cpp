        ...
        if (node["incoming"].IsSequence()) {
            for (std::size_t i=0; i<node["incoming"].size(); i++) {
                ...
                if (UML::isValidUUID4(parsedId)) {
                    ...
                    parseNowOrLater(incomingId, el->getID(), node, &ActivityNodeParser::addIncomingEdgeLater);
                } else {
        ...
        if (node["outgoing"].IsSequence()) {
            for (std::size_t i=0; i<node["outgoing"].size(); i++) {
                ...
                if (UML::isValidUUID4(parsedId)) {
                    ...
                    parseNowOrLater(outgoingId, el->getID(), node, &ActivityNodeParser::addOutgoingEdgeLater);
                } else {
                    // TODO edge definitions in nodes?
                }
            }
